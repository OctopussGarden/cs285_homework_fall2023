...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        # What does actor_gradient_type mean?
        # - "reinforce": Uses the REINFORCE algorithm, which is a policy gradient method.
        # explain further
        #   It computes the gradient of the expected return with respect to the policy parameters
        #   and updates the policy in the direction of the gradient.
        #   This method can be noisy and has high variance, but it is simple to implement
        #   and works well for discrete action spaces.
        #   It samples actions from the policy and uses the rewards to compute the gradient.
        #   The policy is updated using the log-probabilities of the actions taken and the
        #   estimated Q-values (or advantages) of those actions.
        # - "reparametrize": Uses the reparameterization trick, which allows for more stable training
        #   by treating the action as a deterministic function of the state and a noise variable.
        ...
        # What are different target critic backup types?
        # - doubleq: Uses one critic to select actions and another to evaluate them.
        # - min: Takes the minimum Q-value from multiple critics to reduce overestimation bias  
        # - redq: Uses a randomized ensemble of critics to improve stability.
        # - mean: Averages the Q-values from multiple critics, which can help reduce variance
        # Soft actor-critic
        ...
        ) # Why use several critics?
        # This is useful for SAC, which uses multiple critics to reduce overestimation bias.
        # Each critic is trained on the same data, but they can learn different estimates of the
        # Q-value function, which helps to stabilize training and improve performance.
        # The final Q-value is typically taken as the minimum or mean of the Q-values from
        # the different critics, which helps to mitigate the overestimation bias that can occur
        # when using a single critic.
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            next_qs[0],next_qs[1] = next_qs[1], next_qs[0]
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            next_action = next_action_distribution.sample() # No gradient, so use `.sample()`
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                next_action_entropy = -self.temperature * next_action_distribution.log_prob(next_action)
                next_qs += next_action_entropy

            # Compute the target Q-value
            target_values: torch.Tensor = reward + self.discount * (~done) * self.discount * (next_qs)
        ...
        # Predict Q-values
        q_values = self.critic(obs=obs, action=action) # Q value using current obs and action
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        return -self.temperature * action_distribution.log_prob(action_distribution.rsample()) # TODO 
    ...
    def actor_loss_reinforce(self, obs: torch.Tensor):
        ...
        loss = -(advantage - self.temperature * log_probs).mean()
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        q_values = self.critic(obs, action).mean(dim=0) # ensemble mean
    ...
    def update(
        ...
        elif self.soft_update_period is not None:
